...
void MyMusicPlayer::showSongs()
{
    ...
    while(query.next())//一行一行的遍历
    {
        ...
        //query.value()
        int m = query.value(4).toInt()/1000/60;
        int s = query.value(4).toInt()/1000%60;
        char *times;
        sprintf(times,"%.2d:%.2d",m,s);
        ui->infoSong->setItem(row,3,new QTableWidgetItem(QString(times)));
        playerList->addMedia(QUrl("file:///"+query.value(0).toString()));
//        qDebug() << "initial=====================" <<endl;
//        qDebug() << QUrl("file:///"+query.value(0).toString()) << endl;


    }
    ui->infoSong->selectRow(0);//设置当前光标所在行
    ui->showNowSong->setText(tr("正在播放: %1").arg(ui->infoSong->item(0, 0)->text()));//初始化现在播放的歌曲名字
     ui->lbding->setText("/"+ui->infoSong->item(0, 3)->text());//初始化时间
...
void MyMusicPlayer::addFile()
{
    ...
    if(tempPlayer.isMetaDataAvailable()){
        ...
        playerList->addMedia(QUrl(filepath));   //如果可以加入数据库再进行添加到播放列表
    }
...
void MyMusicPlayer::prevSong()
{
    ...
    player->play();
    ui->infoSong->selectRow(currentIndex);
    ui->showNowSong->setText(tr("正在播放: %1").arg(ui->infoSong->item(currentIndex, 0)->text()));//设置现在播放的歌曲名字
    //修改歌曲的总时间

    ui->lbding->setText(ui->infoSong->item(currentIndex, 3)->text());
...
void MyMusicPlayer::changeSliderProcess(qint64 duration)
{
    ...
    char times[6];
    sprintf(times,"%.2d:%.2d",m,s);
    ui->lanow->setText(QString(times));
}
